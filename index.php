...
<?php
    ...
    if (!$jsonData) {
        ...
        // Merge new data with existing data based on messageId
        foreach ($newData as $newItem) {
            $found = false;
            foreach ($jsonData as &$item) {
                if ($item['messageID'] === $newItem['messageID']) {
                    $found = true;
                    $item = $newItem; // Update existing item
                    break;
                }
            }
            if (!$found) {
                $jsonData[] = $newItem; // Append new item if messageId doesn't exist
            }
        }
...
<hr>
<h1 style="text-align: center">SMS Reminder Module - Realtime Reports</h1>
<p style="text-align: center">Page Number: <span id="pageNumber"><?php echo $pageNumber; ?></span></p>
...
<div style="align-text: center">
    <button id="prevPage" class="btn btn-primary">Back</button>
    <button id="nextPage" class="btn btn-info">Next Page</button>
</div>
